...
const mapearCategoriaEItem = (categoryId: string): { categoria: 'Offline' | 'Ads' | 'Marketing' | 'Ferramentas' | 'Outros', metric: string } => {
  switch (categoryId) {
...
export const adaptModernToLegacy = (
  ...
  // Detalhe de PF vs PJ (May has 59% PF, other months 5% PF)
  ...
  const organicSource = current.trafficSources.find(s => s.source.toLowerCase().includes('orgânico') || s.source.toLowerCase().includes('outros'));
  ...
  const conversoes: ConversoesData = {
  ...
  // 4. Investimento
  // Calcule o total investido das campanhas de tráfego pago ou da soma total
  ...
  const investimento: InvestimentoData = {
  ...
  let investimentosTabela: InvestmentItem[] = [];

  const monthlyDetail = investmentsData.monthlyDetails[summary.month];
  if (monthlyDetail && monthlyDetail.investments && monthlyDetail.investments.length > 0) {
    investimentosTabela = monthlyDetail.investments
      .map(inv => {
  ...
  // 8. Anúncios
  // Dados de plataformas dinâmicas
  ...
  const anuncios: Record<string, AdPlatformData> = {
    'Google Ads': {
  ...
  // 9. Funil
  // Monta as métricas do funil dinamicamente
  ...
  // 10. Cidades
  // Mapeia o vetor de cidades
  const cidades: CidadeItem[] = current.cityDistribution.map(c => {
    // Busca crescimento relativo anterior ou inventa sutilmente
    let grow = '+ 12,4%';
